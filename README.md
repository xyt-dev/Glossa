...
✨ 逐句对照翻译，卡片式讲解每个生词与地道表达；记住你标记的每一个词，越用越懂你的水平；随时无缝切换大模型对话模式，共享上下文；全平台使用，支持自行部署，随时随地，在你的任何设备上使用.
...
> **同一个 Glossa，桌面、浏览器、手机无缝切换**

**一套 Rust 编写的应用内核 + 一套应用界面**跑在所有平台上：

| 🖥️ **桌面端** | 🌐 **浏览器** | 📱 **手机** |
|:---:|:---:|:---:|
| Linux · Windows · macOS<br/>原生 Tauri 应用，单文件、零依赖 | 一行命令起服务<br/>任何浏览器直接访问 | 电脑部署后，手机浏览器打开即用 |
...
- **🔌 接你自己的模型** —— 支持 **OpenAI-compatible** API，也支持**原生 Anthropic / Claude 协议**（默认 DeepSeek）。API key 存在你自己机器上，对话不经过任何第三方。
