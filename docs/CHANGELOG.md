...
- `kernel::app::AppCore` 抽出 UI-agnostic 业务门面，Tauri commands 与 Web server 共用同一套 session / memory / config / agent 调用。
- 新增 `crates/web-server`（二进制 `glossa-web`），默认只监听 `127.0.0.1:1421`，提供本机 HTTP API、`send_message` SSE 流与 `ui/dist` 静态文件服务。
- 前端 API 抽象为 Tauri IPC 与 HTTP 两条 transport：Tauri WebView 继续用 `invoke` / `Channel`，普通浏览器通过 `/api` + fetch/SSE 访问本机 server。
- Vite dev server 增加 `/api -> http://127.0.0.1:1421` 代理，方便浏览器开发模式调试 Web 入口。
- 验证：`cargo test -p kernel`（30 passed）、`cargo check -p glossa-web`、`cargo check -p glossa`、`npm --prefix ui run build`；使用临时 XDG 目录启动 `cargo run -p glossa-web` 后，`GET /api/sessions` 返回 `[]`，`GET /` 返回 `200 OK text/html`。
