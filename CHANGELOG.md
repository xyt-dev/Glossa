...
- **前端内嵌折叠段**（`Reasoning.tsx`）：思考是**回复气泡内顶部**的可折叠段（同一个框、虚线
  ...
  - **折叠状态由 `streaming` 派生**（`open = 用户手动 ?? streaming`），不靠捕捉 true→false 跳变——
    只要不在思考中就折叠，与挂载时机无关（修「偶尔思考与正文没分开」）；思考完成用 `useLayoutEffect`
    在正文绘制前**强制回到自动=折叠**，之后点标题随时展开/收起。
  - 思考内容**不做内部滚动**（不限高），随整个对话区一起滚，避免嵌套滚动卡住滚轮。
  - 折叠状态**每会话独立**：历史 turn 的 React key 带会话 id（只用数组下标会让切会话按位置复用
    旧实例、把折叠状态串到别的会话）。
- **spinner**：`will-change: transform`（流式主线程繁忙也在合成层顺畅转，不再冻住）；三边透明、
  只留一段 accent 圆弧（去掉拼接不均、偏深的灰色轨道）；`aspect-ratio/box-sizing/flex:none` 保正圆。
