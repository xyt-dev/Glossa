...
每个 section 都有自己的标题和轻提示文案。这样做的目的不是“加说明文字”，而是把结构层次明确写进 DOM 和样式里，避免
以后继续靠一堆 `margin` / `nth-child` / 临时 class 去修感觉，降低后续 UI 演化时长成屎山的风险。

### 句子译文弹层重构：更宽、更稳、更像批注

句子点击后的 Popover 默认宽度从 560 提高到 720，并同步补齐手机端约束：
- 桌面端允许更舒展的译文展示，不再显得局促；
- 手机端明确限制 `min/max-width` 与 `max-height`，避免“桌面放宽后移动端炸出屏幕”。

同时把弹层内容从“原文一行 + 译文一行”改成更清晰的两段式结构：
- 原文
- 译文

每段都有轻量 label，读起来更像页内注释 / 批注，而不是工具弹窗塞两段文本。
