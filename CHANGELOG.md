...
这一版不加新能力，主要是把**翻译模式的展示层次与弹层交互做了一轮重构**，目标是让翻译内容在同一块窗口里
更稳定、更清楚地承载长文本学习场景，同时尽量不把复杂度堆回上层对话逻辑。

### 翻译结果布局重构：弱化气泡感，更充分利用窗口空间

原先翻译结果虽然功能完整，但外观仍然沿用聊天气泡的视觉语言：宽度偏保守、圆角不对称、结构更像“聊天里塞了一个
翻译组件”。长段翻译时，内容的阅读感和空间利用都不够理想。

这次把翻译块整体改成更中性的内容面：
- 宽度显著放宽，优先利用主窗口空间，而不是继续保持聊天气泡那种 90% 左右的保守宽度；
- 圆角、阴影、边框改得更对称，弱化“消息气泡”特征；
- 留白重新分配，让翻译结果读起来更像一页内容，而不是一条消息。

### 翻译区结构重构：从组件平铺改成明确 section

之前 `TranslationBlock` 的 DOM 结构偏扁平：句子区、词卡区、用法卡区直接顺序堆叠，功能都在，但层次更像“组件列表”而不
像一块完整内容。
...
现在只保留 section 标题，不再加解释文案；这样既把结构层次写进 DOM 和样式里，又避免翻译区看起来像一块“功能说明面板”。

### 原文阅读区重构：保留用户输入里的换行与空行

原先原文区直接用模型返回的 `sentences[].src` 平铺。这样做虽然便于句子级点击，但会丢失用户输入里的：
- 换行
- 空行
- 段落边界
- 原始空白节奏

现在改成：原文展示优先使用**用户真实输入文本**作为底稿，再把 `sentences[].src` 映射回这份原文里做句子级点击。
这样一来：
- 句子仍然可以点开看译文；
- 原文的段落和阅读节奏按用户输入原样保留。

如果句子映射失败（比如模型回吐的 `src` 与原文略有差异），则安全回退到旧的句子平铺方式，不会把交互弄坏。

### 句子译文弹层重构：更宽、更稳、更像批注

句子点击后的 Popover 默认宽度从 560 提高到 720，但桌面和手机不再共用一套“固定宽度”逻辑：
- 桌面端允许更舒展的译文展示，不再显得局促；
- 手机端按**当前视口宽度自适应**，尽量吃满可用空间，同时始终保留安全边距，而不是简单回退到一个固定窄宽度。

同时弹层定位模型也重写了：不再用“锚点中心 + `translateX(-50%)`”去推本体，而是直接计算**左边缘**并据此反算箭头位置。
这样能避免长句弹层在手机端向左漂出窗口、以及本体与菱形箭头分离的问题。

内容结构上，也从“原文一行 + 译文一行”改成更清晰的两段式：
- 原文
- 译文

每段都有轻量 label，而且原文与译文现在采用**同一字号体系**，读起来更像页内注释 / 批注，而不是工具弹窗塞两段不同等级的文本。


### 学习卡片样式重构：保留信息量，降低组件味

词卡 / 用法卡没有改语义和交互，只做了展示层收敛：
- 背景更轻，边框和强调线减弱；
- examples 的左线和行距更像阅读批注，不再那么像功能模块；
- 收藏按钮继续弱化成轻标签式操作，减少按钮存在感，避免和正文争抢视觉重心。

换句话说，这一轮是把学习卡片从“工具盒子”往“阅读辅助层”推了一步。

### 交互边界保持不变：只重构翻译子树，不把复杂度堆回 Conversation

这轮刻意不碰：
- `Conversation.tsx` 的滚动 / streaming / pendingUser 逻辑
- `ChatBubble.tsx` 的聊天路径
- API / 持久化 / 数据结构

所有改动都被限制在翻译子树与样式层：`TranslationBlock`、`SentenceView`、`Popover`、相关 CSS。这样做是为了把“翻译区像一页内容”
这个问题限制在翻译区内部解决，不把上层消息流和滚动逻辑一起卷进来，避免 UI 打磨反过来污染核心会话逻辑。

### 关于 GTK `border-spacing` warning

顺手排查了运行时出现的：

> `Gtk-WARNING **: gtk-dark.css:1048:16: 'border-spacing' is not a valid property name`

结论是：仓库 `ui/src` 与构建产物 `ui/dist` 中都**没有** `border-spacing`，所以这不是本项目样式直接写出来的属性，更像是系统 GTK / WebKitGTK 主题层的 warning，而不是当前翻译区代码引入的应用层 CSS 问题。本次重构没有把它“修没”，但至少已经确认它不是我们样式表里的显式回归。

## v0.5.1 - 2026-07-06

本版把 Glossa 从“只会说 OpenAI-compatible 协议”扩成了**双协议客户端**：继续支持现有
`/chat/completions` 生态，同时新增 **Anthropic / Claude 原生 `/v1/messages`** 支持。重点不是
“多塞几个 if”，而是把**协议层差异**和**上层语义**拆开：协议怎么发、怎么收，各走各的；而
`agent.rs`、思考 UI、会话存储继续只认统一的正文 / thinking 语义。

### 新增：Anthropic / Claude 原生协议支持

**修复前**：虽然能接很多 Claude 中转站，但前提是它们提供的是 **OpenAI-compatible**
`/chat/completions`。如果一个站只暴露 **Anthropic 原生** `/v1/messages`，当前客户端就接不上。

**为什么不能像 OpenAI-compatible 一样只换个 base_url？** 因为协议不是一套：
- OpenAI-compatible：`/chat/completions`、`Authorization: Bearer`、`choices[].delta.*`
- Anthropic-native：`/v1/messages`、`x-api-key + anthropic-version`、content blocks / SSE events

所以这次不是“再猜一个 URL”，而是把 provider 提升成**协议选择器**：
- `openai`：OpenAI-compatible `/chat/completions`
- `deepseek`：仍走 `/chat/completions`，保留 DeepSeek thinking 兼容层
- `anthropic`：原生 `/v1/messages`

### 兼容原理：先判协议，再统一语义

后端 `client.rs` 现在按协议拆成几组 helper：
- endpoint
- headers
- request body
- non-stream parser
...
**上层保持不变**：`agent.rs::run_turn()` 仍然只消费统一的 `Delta::{Reasoning, Content}`；
前端 reasoning UI、`Message.reasoning` 持久化、`SendEvent` 流式事件也全部复用。

也就是说：
> 协议不同，但上层只看“这是思考还是正文”。

这样新增 Anthropic 路径时，不需要把 translate/chat、repair/fallback、thinking UI 再重写一遍。

### Anthropic 请求如何映射

Anthropic provider 下改成：
- endpoint：`{base_url}/v1/messages`
- headers：`x-api-key` + `anthropic-version: 2023-06-01`
- body：
  - top-level `system`（从现有 `ChatMessage::system` 提取）
  - `messages`
  - 必填 `max_tokens`
  - 有思考时发 `thinking: {type:"adaptive", display:"summarized"}`
  - translate 模式下改用 **`output_config.format` + JSON Schema** 做结构化输出

**为什么这里比 OpenAI-compatible 更“啰嗦”？**
- 不是项目突然变复杂，而是 Anthropic 原生协议本来就不是 OpenAI 那套字段名/路径
- 严谨起见，这里没有走“继续 prompt 它吐一段 JSON 文本”那种脆弱路线，而是显式给 JSON Schema

### Anthropic relay 兼容原理：严格原生优先，再宽松吃常见变形

很多 Claude relay 看起来是 `/v1/messages`，但流式返回并不一定完全原样 Anthropic：有的会把
thinking 吞掉，有的会把字段改成 OpenAI-like 形状，有的会混用不同事件类型。这也是为什么用户会遇到
“**聊天模式 thinking 有时显示、有时不显示**”。
...
2. **再兼容常见 relay 变形**
   - `content_block_start` 里直接带 `thinking` / `text`
   - OpenAI-like `choices[].delta.reasoning`
   - OpenAI-like `choices[].delta.reasoning_content`
   - OpenAI-like `choices[].delta.content`
   - `content_block_delta` 外壳下但内部字段名不标准的 `reasoning` / `content` / `text`

这样做的原则是：
> **先保证原生协议正确，再尽量兼容 relay 的“半原生”实现。**

而且我**没有继续按 host 名乱猜协议**：只自动识别 Anthropic 第一方 `api.anthropic.com`，第三方中转站
URL 往往看不出底层协议，应该显式选 `provider = "anthropic"`，否则误判比“不猜”更危险。

### 配置 / UI / 文档同步更新

为了让用户明确自己在走哪条协议链路：
- 配置模板不再写“任意 OpenAI 兼容 API 均可”，改成明确说明同时支持
  **OpenAI-compatible 与 Anthropic-native**
- `Profile.provider` 注释扩成 `deepseek | openai | anthropic`
- 设置界面新增 **`Anthropic / Claude 原生`** 选项，标题从“Provider 兼容层”改成
  **“Provider 协议层”**
- `Temperature` 标签注明：**Anthropic 原生协议不使用**
- README 同步写清“支持 OpenAI-compatible，也支持原生 Anthropic / Claude 协议”

另外，Anthropic-native 下不会像 OpenAI-compatible 那样“任意 extra 字段原样透传”，而是只透传一小组
**白名单字段**（如 `max_tokens` / `metadata` / `stop_sequences` / `service_tier`），避免把 OpenAI
参数误塞给 Anthropic 导致 400 或行为不明。
...
1. **`client.rs` 协议层单测**
   - endpoint / header / body 构造
   - Anthropic 非流式正文提取
   - Anthropic 严格原生流式 thinking/text 解析
   - relay 变形流式解析（`content_block_start` / `reasoning_content` / `reasoning` 等）

2. **`agent.rs` 端到端测试**
   - Anthropic chat turn
   - Anthropic translate turn
   - Anthropic repair / fallback
   - reasoning 单独流出并单独持久化

结果：`cargo test -p kernel` 通过，**41 passed**。

### 小修：模式角标不可被选中

消息气泡里的「译 / 聊」只是模式标识，不属于正文；加 `user-select: none` /
`-webkit-user-select: none`，拖选/复制消息文本时不再把角标一起选中。

## v0.5.0 - 2026-07-06

本版围绕**多会话并行 + 流式体验**重做了前端「正在生成」状态，并新增**模型思考过程**展示。
后端、数据结构、会话存储格式基本未变（仅 `Message` 新增可选 `reasoning` 字段，向后兼容）。

### 多会话并行生成，界面不再被单条消息卡住

**现象（修复前）**：发出一条消息后，在它生成完之前整个界面都卡死——点不了侧边栏、切不了
会话、也建不了新会话，只能干等。

**原因**：前端把「正在生成」做成了**一个全局开关**（`busy` / `streamText` / `streamMode`）。
一发消息 `busy=true`，切换 / 新建会话的函数里都 `if (busy) return` 直接拦掉，侧边栏按钮和
输入框也全 `disabled`。其实后端每条消息本就各跑各的（独立 tokio 任务、各写各的会话文件），
不同会话天生能并行，卡死纯粹是前端这个全局开关造成的。

**改法**：全局开关换成**按会话隔离**的 `streams: Record<会话id, {text, reasoning, mode, user}>`，
每个会话有各自的「正在生成」状态：
- 「正在生成」只禁用**当前会话**的输入框，侧边栏永远可点；生成期间可随意切换 / 新建会话，
  后台会话继续生成，侧边栏对应条目用小转圈代替时间标明它在跑。
- **切走再切回不丢消息**：本轮问的问题也存进该会话进行中状态，由对话区据此渲染并与后端已存
  消息去重——即便生成中切走再切回、或界面回读早于后端落盘，问题气泡也不消失。

**边界**（有意保留）：同一会话仍不允许并发发送（后端对单会话是「读—改—写」，并发会互相覆盖
历史，靠当前会话忙碌态禁用输入兜底）；错误提示暂仍是全局横幅；暂无「停止生成」。

### 流式性能：为什么要拆分渲染、怎么拆的

**为什么需要**：流式回复每来一个 token 就 `setState` 一次 → 整个 `App` 重渲染 → 所有子组件
跟着重渲染。其中两处代价最大：
- **侧边栏**每 token 把所有会话 tab 重新渲染一遍（纯浪费，tab 内容根本没变）；
- **对话区**每 token 把**全部历史消息**重新 map、每条 `ChatBubble` 都重新跑一遍 react-markdown
  解析——会话越长越卡。

**怎么拆**（把「每 token 必变的东西」和「不变的东西」分开，让不变的跳过重渲染）：
- 给 `Sidebar` / `InputBar` 套 `memo`，props 引用稳定时直接跳过重渲染；并让「哪些会话在
  生成」这个集合的**身份只在成员变化时才变**（不随思考/正文文字每 token 变），否则 `memo`
  会被新身份击穿。→ 生成期间侧边栏、输入栏不再随每个字重渲染。
- 用 `useMemo` 把**历史消息列表**隔离出来：流式期间 `active` 不变 → 列表元素引用不变 →
  React 跳过所有历史气泡，**每个 token 只重画底部那条正在生成的气泡**，不再逐条重解析 markdown。

结果：流式期间的重渲染从「全界面 × 每 token」收敛到「一条气泡 × 每 token」；长会话尤其明显。

**再加一层节流**：token 先攒进缓冲，每 ~60ms（≈16fps）才 `setState` 一次落到界面，而不是每
token 一次。流式文本可瞥读、不需要 60fps；这样把「每帧重排+重渲染」的主线程压力再砍数倍，
副作用是让挨着流式文字的 CSS 动画（思考转圈）不被主线程饿死——见下「思考转圈卡顿」。

### 回复生成时可以往上滚动了

以前回复每来一个字，对话区都强行把视图拽回底部，生成中根本没法往上翻历史。改成**只有你本来
就贴着底部时才自动跟随**；一旦往上滚看历史就不再拽你，滚回底部附近又恢复跟随。关键坑：程序化
「滚到底」自身会触发一次 `scroll` 事件，若不区分就会在你上滑生效前把「贴底」状态重置真——用
`selfScrollRef` 标记忽略程序化滚动、只认用户滚动。切换会话仍直接跳到底部看最新。

`selfScrollRef` **必要但不够**：per-token 高频下，跟随每秒触发几十次，两个隐患——① 频率上压制，
你刚上滑一点下个 token 就把你拽回；② `scroll` 事件异步，你的上滑事件可能先到、被 `selfScrollRef`
当成程序化滚动**误吃**掉，`pinned` 没解除。上面「流式性能」的 ~16fps 节流顺带补上了这一环：跟随
频率骤降、`selfScrollRef` 窗口又小又稀，你的滚动才稳稳生效。**转圈卡顿和滚动卡底是同一个根
（per-token 更新风暴），一个节流一起治了。**

### 显示模型「思考过程」（可折叠、内嵌在回复里）

推理模型（如 DeepSeek `chat_effort=xhigh`）出答案前会先思考很久，而之前我们**丢弃**了思考内容
（`chat_stream` 只取 `content`）——思考期就是个不动的转圈、零输出。多开对话时每个都不吐字，
看着像「一个等另一个输出完」，其实一直并行，只是思考被藏了。现在把它显性化：

- **后端分流**：`client.rs::chat_stream` 改为分别产出 `Delta::Reasoning` 与 `Delta::Content`
  （兼容 DeepSeek `reasoning_content` 与聚合商 `reasoning` 字段）；`agent.rs` 分开累积，思考走
  新的 `SendEvent::Reasoning` 事件，**绝不混入**被解析/保存的答案，并持久化到 `Message.reasoning`
  （旧会话无此字段，`serde(default)` 兼容）。桌面（Tauri Channel）与 Web（NDJSON）两个适配层
  都是泛型序列化 `SendEvent`，加事件变体两端透明。
- **前端内嵌折叠段**（`Reasoning.tsx`）：思考是**回复气泡内顶部**的可折叠段（同一个框、虚线
  分隔），不是独立盒子；**仅聊天模式展示**（翻译模式即使开了思考也不显示）。
  - **默认折叠、只由点击切换**（无自动展开/自动折叠）：思考期 header 显示「思考中…」并在文字
    前带转圈，点开即看流式思考文本，不点就一直折着——正文照常在下面出。想看就点、不想看不打扰。
  - 思考内容**不做内部滚动**（不限高），随整个对话区一起滚，避免嵌套滚动卡住滚轮。
  - 折叠状态**每会话独立**：历史 turn 的 React key 带会话 id（只用数组下标会让切会话按位置复用
    旧实例、把折叠状态串到别的会话）。
- **spinner 外观**：三边透明、只留一段 accent 圆弧（去掉拼接不均、偏深的灰色轨道）；
  `aspect-ratio/box-sizing/flex:none` 保正圆。header 里顺序为「小三角 → 转圈 → 文字」，
  小三角改用 **CSS 边框绘制**（不是字体 `▸` 字形——Nerd Font 字形垂直居中不可控），配
  `align-items:center` 与文字精准对齐，展开时旋转 90° 指向下。
- **思考转圈卡顿（原理记录）**：现象是**聊天**思考转圈「跳跳卡卡、hover 直接冻住」，而**翻译**
  的转圈却很顺——两者是**同一套 spinner CSS**，差别在旁边的东西：翻译框旁是**静态**「翻译中…」
  （DOM 不变、不重排），聊天框旁是**逐字增长的思考文本**（每帧改 DOM + 重排）。挨着动画的这块
  持续重排会占满主线程，把没上 GPU 的动画「饿死」；hover 的一次重绘再把合成层踢回主线程 → 冻住。
  两条腿修：① **减负**——流式更新节流到 ~16fps（见上「流式性能」），每帧重排大幅减少；
  ② **隔离**——spinner 用**关键帧内 `translateZ(0)`** 固定在一个稳定合成层由 GPU 独立旋转
  （比 `will-change` 稳，避免 WebKitGTK 反复升降层跳帧），并 `pointer-events:none` 让光标
  不命中它、不触发 hover 重绘。GPU 可用时靠隔离、软件渲染时靠减负，都指向「别让增长的文字拖累转圈」。
- **模式角标不可选中**：消息气泡里的「译 / 聊」只是模式标识，不属于正文；加 `user-select: none`
  / `-webkit-user-select: none`，拖选/复制消息文本时不再把角标一起选中。
...
- **system prompt 按模式拆开**：共用一个只讲角色 + 软件用法的前缀，翻译只带翻译契约、
  聊天只带聊天契约（生词本 memory 两者共享）——提示词互不干扰、更聚焦、更省 token。
- **历史上下文按当前模式过滤**：翻译只回放之前的翻译回合，且 assistant 只回放纯译文
  （跳过聊天噪音、不再塞词卡 JSON），保留术语/风格一致性又大幅降 token；聊天保留完整
  上下文（翻译回合回放完整 JSON，可引用词卡）。新增独立配置 `translate_context_messages`
  （默认 12，翻译窗口宜小），设置面板拆成「聊天 / 翻译上下文窗口」两项。
- **翻译约束增强**：只翻译本轮输入，历史仅供术语/指代/风格参考，严禁翻译、续写、解释历史内容。
- **查词优先**：输入本身是单个单词 / 短语 / 中文词时视为"查词"，重点把它讲透（词卡或 native
  用法卡），无视 band 下限与数量限制。
- **前缀内置软件用法说明**：给模型一份面向用户的界面操作手册，用户问"怎么用 / 有什么功能"时
  能从使用角度正确指导（输入框、翻译/聊天切换、生词本收藏……），且不暴露标签 / JSON 等内部实现。
